...
function bootstrapHistorionFromURL(a){$.ajax({url:a,method:"get"}).done(function(a){success("A persona has been loaded successfully. Wait as it gets rendered...");"string"===typeof a||a instanceof String?loadNewPersona(JSON.parse(a)):loadNewPersona(a)}).fail(function(){error("Fetching the persona from the URL has failed! Can't bootstrap this Historion. Try Reseting...")})}
...
function showGettingStarted(){$("#app-title").text("The (Web) Historion").fitText();$(document).prop("title","The (Web) Historion");var a=$("#app-base"),c=$("#app-body");a.css({"background-color":"#fff"});a.find(".panel").css({"background-color":"#fff"});a.find(".panel-heading").css({"background-color":"#f5f5f5",color:"#333","text-shadow":"gray 3px 3px 3px"});$("body").css({"background-color":"#f5f5f5"});a.find(".panel").css({"border-color":"#f5f5f5"});c.css({color:"inherit"});c.empty();$("#btnSave,#btnSubmit").hide();
info("It's too lonely here ;-) <br/>Perhaps, go to the <b>PERSONAS</b> section and import an app...<hr/>Alternatively, read more about the Historion, and all of <b><a href='https://nuscribes.com/scribes_app/book/40/read/?#chapter-500' target='_blank'>Project-Okot, on NuScribes.</a></b>")}function acts(){var a=localStorage.getItem(DB_KEYS.SAVED_ACTS);null==a?info("There are no acts (saved data) in the local cache."):a=JSON.parse(a);renderActsManagement(a)}
...
b=$("<div/>",{"class":"input-group"}).append(g,$("<span/>",{"class":"input-group-btn"}).append(d));f.append(b);d.click(function(){var a=g.val().trim();0==a.length?error("Can't import from a blank URL!",!0):(info("Please wait as I try to fetch a persona from the specified URL..."),bootstrapHistorionFromURL(a))});d=$("<input/>",{"class":"form-control input-lg",type:"file",title:"Pick a *.persona file or other, from which to import from"});b=$("<div/>",{"class":"form-group"}).append(d);f.append(b);d.on("change",
